#include "utils.h"
#include <Psapi.h> // used for MODULEINFO and some other stuff
#include <stdlib.h>
#include <stdio.h>
#include <io.h>
#include <fcntl.h>
...
namespace mem
{
	...
	// sometimes some pointers will be pointing to invalid memory and by using this function we can prevent crashes
	__forceinline uintptr_t GetPointerSafe(uintptr_t base, const std::vector<uintptr_t>& offsets)
	{
		for (const uintptr_t& offset : offsets)
		{
			base = readSafe<uintptr_t>(base);
			if (!base) return 0;
			base += offset;
		}
		return base;
	...
	const char* GetGName(const int FNameIndex)
	{
		...
		uintptr_t chunk = GetPointerSafe(GNames + (FNameIndex / SIZE_GNAME_CHUNK) * sizeof(void*), { 0x0 });
		if (!chunk)
			return "";

		uintptr_t nameptr = GetPointerSafe(chunk + (sizeof(void*) * (FNameIndex % SIZE_GNAME_CHUNK)), { 0x0 });
		if (!nameptr)
			return "";
		...
		//static const uintptr_t GNames = *(uintptr_t*)(ModuleBase + GNamesOffset);
		//if (!GNames || !GetPointerSafe(GNames + (FNameIndex / SIZE_GNAME_CHUNK) * sizeof(void*), { 0x0 }))
		//	return "";

		//uintptr_t chunk = *(uintptr_t*)(GNames + (FNameIndex / SIZE_GNAME_CHUNK) * sizeof(void*));
		//if (!chunk || !GetPointerSafe(chunk + (sizeof(void*) * (FNameIndex % SIZE_GNAME_CHUNK)), { 0x0 }))
		//	return "";
